...
export const POST: APIRoute = async ({ request }) => {
  ...
  const concern = (data.get('concern') as string | null)?.trim() ?? '';
  ...
  const { error } = await resend.emails.send({
    ...
    to: [toEmail],
    replyTo: email,
    subject: `Patient Concern from ${name}`,
    html: `
      <h2>Patient Concern Submission</h2>
      <p><strong>Name:</strong> ${name}</p>
      <p><strong>Email:</strong> ${email}</p>
      <p><strong>Concern:</strong><br>${concern || '(no details provided)'}</p>
    `,
  });
